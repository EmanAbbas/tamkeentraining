...
;( function( $, window, undefined ) {
	...
	$.PMSlider.prototype = {
		...
		_animateSlide : function(currSlide) {
			...
			if(title.length > 0){
				$slide.find('h1').animate({
					'width' : $slide.data('properties').titleWidth + 'px',
					'padding' : '10px 15px',
					'opacity' : 1,
				},760,"swing", function(e) {
					if(caption.length > 0){
						$slide.find('.pm-caption-decription').animate({
							'width' : $slide.data('properties').captionWidth + 'px',
							'padding' : '13px',
							'opacity' : 1,
						},480,"swing", function(e) {
							if(btn.length > 0){
								btn.animate({
									'opacity' : 1,
									'marginLeft' : '0px'	
		...
		_resetAnimation : function() {
			...
			this.$items.each(function(i, el) {
				...
				if(caption.length > 0){
					$el.find('.pm-caption-decription').css({
						'width' : '0px',
						'padding' : '13px 0px',
						'opacity' : 0,	
					});
		...
		_toolTip : function(toolTipData, el){
			...
			$('#pm_slider_tooltip').css("top", ($el.offset().top - yOffset) + "px").css("left",+ ($el.offset().left - xOffset) + "px"); //start position
